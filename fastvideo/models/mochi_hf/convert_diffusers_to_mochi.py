...
def convert_diffusers_vae_to_mochi(state_dict):
    ...
    for block in range(3):
        ...
        encoder_state_dict[f"layers.{block + 4}.layers.0.bias"] = (
            original_state_dict.pop(f"{prefix}down_blocks.{block}.conv_in.conv.bias")
        )
    ...
    # Convert block_in
    for i in range(3):
        decoder_state_dict[f"blocks.0.{i + 1}.stack.0.weight"] = (
            original_state_dict.pop(
                f"{prefix}block_in.resnets.{i}.norm1.norm_layer.weight"
            )
        )
        ...
        decoder_state_dict[f"blocks.0.{i + 1}.stack.2.weight"] = (
            original_state_dict.pop(f"{prefix}block_in.resnets.{i}.conv1.conv.weight")
        )
        decoder_state_dict[f"blocks.0.{i + 1}.stack.2.bias"] = original_state_dict.pop(
            f"{prefix}block_in.resnets.{i}.conv1.conv.bias"
        )
        decoder_state_dict[f"blocks.0.{i + 1}.stack.3.weight"] = (
            original_state_dict.pop(
                f"{prefix}block_in.resnets.{i}.norm2.norm_layer.weight"
            )
        )
        ...
        decoder_state_dict[f"blocks.0.{i + 1}.stack.5.weight"] = (
            original_state_dict.pop(f"{prefix}block_in.resnets.{i}.conv2.conv.weight")
        )
