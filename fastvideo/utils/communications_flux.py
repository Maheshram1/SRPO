    ...
    while True:
        for data_item in dataloader:
            ...
            if frame == 1:
                ...
                # Slice from the prepared batch each time; do not progressively shrink
                base_encoder_hidden_states = encoder_hidden_states
                base_pooled_prompt_embeds = pooled_prompt_embeds
                base_text_ids = text_ids
                base_caption = list(caption) if isinstance(caption, (list, tuple)) else caption
                assert train_batch_size * sp_size >= train_sp_batch_size, (
                ...
                for iter in range(train_batch_size * sp_size // train_sp_batch_size):
                    ...
                    encoder_hidden_states = base_encoder_hidden_states[st_idx:ed_idx]
                    pooled_prompt_embeds = base_pooled_prompt_embeds[st_idx:ed_idx]
                    text_ids = base_text_ids[st_idx:ed_idx]
                    caption_slice = (
                        base_caption[st_idx:ed_idx]
                        if isinstance(base_caption, list)
                        else base_caption
                    )
                    if encoder_hidden_states.numel() == 0:
                        # Skip empty slices (can happen when the last chunk is smaller)
                        continue
                    yield (
                        encoder_hidden_states,
                        pooled_prompt_embeds,
